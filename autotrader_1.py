...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            ...
            if (sav - sbd) > 0.05 * (sav + sbd):
                order_type = Lifespan.FAK
            else:
                order_type = Lifespan.GFD
            ...
            if self.bid_id == 0 and new_bid_price != 0 and self.position <= (POSITION_LIMIT - LOT_SIZE):
                ...
                self.send_insert_order(
                    self.bid_id, Side.BUY, new_bid_price, LOT_SIZE, order_type)
            ...
            if self.ask_id == 0 and new_ask_price != 0 and self.position >= -(POSITION_LIMIT - LOT_SIZE):
                ...
                self.send_insert_order(
                    self.ask_id, Side.SELL, new_ask_price, LOT_SIZE, order_type)
